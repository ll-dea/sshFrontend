...
namespace SSH_FrontEnd.Services
{
    public class PastryShopService : BaseServices, IPastryShopService
    {
        ...
        public PastryShopService(IHttpClientFactory clientFactory, IConfiguration configuration) : base(clientFactory)
        {
        ...
        public Task<T> CreateAsync<T>(PastryShopDTO dto)
        {
            return SendAsync<T>(new APIRequest { ApiType = SD.ApiType.POST, Data = dto, Url = _url });
        }

        public Task<T> UpdateAsync<T>(PastryShopDTO dto)
        {
            return SendAsync<T>(new APIRequest { ApiType = SD.ApiType.PUT, Data = dto, Url = $"{_url}/{dto.ShopId}" });
        }
