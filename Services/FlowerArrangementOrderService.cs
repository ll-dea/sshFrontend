...
{
    public class FlowerArrangementOrderService : BaseServices, IFlowerArrangmentOrderService
    {
        ...
        public Task<T> CreateAsync<T>(FlowerArrangementOrderDTO dto)
        {
            return SendAsync<T>(new APIRequest { ApiType = SD.ApiType.POST, Data = dto, Url = _url });
        }

        public Task<T> UpdateAsync<T>(FlowerArrangementOrderDTO dto)
        {
            return SendAsync<T>(new APIRequest { ApiType = SD.ApiType.PUT, Data = dto, Url = $"{_url}/{dto.FlowerArrangementOrderId}" });
        }
